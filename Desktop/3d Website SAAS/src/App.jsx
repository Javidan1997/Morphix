...
const DEFAULT_HERO_PRODUCT = "pergola";
const defaultHeroSelections = {
  pergola: { size: "standard", finish: "charcoal", feature: "lighting", environment: "outdoor" },
  sofa: { size: "standard", finish: "graphite", feature: "chaise", environment: "living-room" },
  light: { size: "medium", finish: "brass", feature: "warm", environment: "dining" },
};
...
function HeroConfiguratorPreview({ product, selections }) {
  const finish = getHeroOption(product, "finish", selections.finish);
  const tone = finish?.color ?? "#314c67";
  const scaleMap = {
    pergola: { compact: 0.88, standard: 1, custom: 1.14 },
    sofa: { compact: 0.88, standard: 1, sectional: 1.16 },
    light: { small: 0.8, medium: 1, large: 1.16 },
  };

  const previewStyle = {
    "--product-tone": tone,
    "--product-scale": scaleMap[product.key]?.[selections.size] ?? 1,
  };

  if (product.key === "pergola") {
    return (
      <div className={`preview-stage preview-${product.key} env-${selections.environment}`} style={previewStyle}>
        <div className="preview-caption">{product.previewNote}</div>
        <div className="preview-pergola">
          <span className="preview-pergola-leg leg-a" />
          <span className="preview-pergola-leg leg-b" />
          <span className="preview-pergola-leg leg-c" />
          <span className="preview-pergola-leg leg-d" />
          <span className="preview-pergola-beam beam-top" />
          <span className="preview-pergola-beam beam-left" />
          <span className="preview-pergola-beam beam-right" />
          <span className="preview-pergola-roof" />
          {selections.feature === "screens" ? <span className="preview-pergola-screen" /> : null}
          {selections.feature === "lighting" ? <span className="preview-pergola-light" /> : null}
          {selections.feature === "fans" ? <span className="preview-pergola-fan" /> : null}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`preview-stage preview-${product.key} env-${selections.environment}`} style={previewStyle}>
        ...
        <div className="preview-sofa">
          <span className="preview-sofa-seat" />
          <span className="preview-sofa-back" />
          <span className="preview-sofa-arm arm-left" />
          <span className="preview-sofa-arm arm-right" />
          {selections.feature === "chaise" ? <span className="preview-sofa-chaise" /> : null}
          {selections.feature === "ottoman" ? <span className="preview-sofa-ottoman" /> : null}
          {selections.feature === "table" ? <span className="preview-sofa-table" /> : null}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`preview-stage preview-${product.key} env-${selections.environment}`} style={previewStyle}>
      <div className="preview-caption">{product.previewNote}</div>
      <div className="preview-light-product">
        <span className="preview-light-cord" />
        <span className="preview-light-shade" />
        <span className={`preview-light-glow glow-${selections.feature}`} />
      </div>
    </div>
  );
}
                ...
                <div className="glass-card hero-configurator-shell reveal" style={{ transitionDelay: "0.12s" }}>
                  ...
                  <div className="hero-shell-body">
                    <div className="configurator-stage">
                      ...
                      <HeroConfiguratorPreview product={activeHeroDemo} selections={activeHeroValues} />
                    </div>
